...
| ID | Status | Cat | Task | Depends |
|---|---|---|---|---|
| P1-001 | `[x]` | `setup` | Add `ANTHROPIC_API_KEY` to `.env.example` | — |
| P1-002 | `[x]` | `setup` | Add `AZURE_DEVOPS_TOKEN` to `.env.example` | — |
| P1-003 | `[x]` | `setup` | Add `AZURE_DEVOPS_ORG` and `AZURE_DEVOPS_PROJECT` to `.env.example` | — |
| P1-004 | `[x]` | `permissions` | Add `anthropic-api-key` credential to `wrappers.yml` (source: `env:ANTHROPIC_API_KEY`) | P1-001 |
| P1-005 | `[x]` | `permissions` | Add `azure-devops-token` credential to `wrappers.yml` (source: `env:AZURE_DEVOPS_TOKEN`) | P1-002 |
| P1-006 | `[x]` | `permissions` | Verify `gh pr create`, `gh issue list`, `gh issue view` are NOT blocked in `wrappers.yml`; add explicit allow-note comment | — |
| P1-007 | `[x]` | `docker` | Extend `openclaw_run` injection script in `AGENTS.md` to also write `ANTHROPIC_API_KEY` and `AZURE_DEVOPS_TOKEN` | P1-001, P1-002 |
...
| ID | Status | Cat | Task | Depends |
|---|---|---|---|---|
| P6-001 | `[x]` | `permissions` | Add `azure-devops-cli` tool entry (uses `az devops` or `azure-devops-node-api` CLI wrapper) with `AZURE_DEVOPS_TOKEN` injected and destructive operations blocked | P1-005 |
| P6-002 | `[x]` | `permissions` | Confirm `gh pr create` and `gh issue list/view/comment` are permitted; add inline policy comment | — |
| P6-003 | `[x]` | `permissions` | Add `anthropic-api-key` credential entry to `wrappers.yml` (for injection into `agent-runner` via `openclaw_run`) | P1-004 |
| P6-004 | `[x]` | `permissions` | Document branch naming convention in `AGENTS.md`: agent PRs must use `agent/<issue-number>-<slug>` | — |
